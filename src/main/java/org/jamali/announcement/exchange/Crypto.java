 ...
 *
 */
public class Crypto extends Exchange {

	public Crypto(Client client, Properties properties) {
		super(client, properties);
		setCondition("Official Blog");
	}
	...
	public String request() {
		...
		try {
			...
			return this.getClient().doGet(this.getUrl(), "https://blog.crypto.com", headers).getResponse();
		} catch (Exception e) {
			return null;
		}
	}
	...
	public HashSet<Model> parse(String announcements) {
		...
		Elements news = doc.select(".post-card-content-link");
		for (Element e : news) {
			models.add(new Model("https://blog.crypto.com" + e.attr("href"), e.select(".post-card-title").text()));
		}
		return models;
	}
